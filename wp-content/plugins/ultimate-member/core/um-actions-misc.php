	...
	***/
	...
	function um_add_update_notice($args){
		...
		if(isset($_GET['code'])) {
			...
			$wp_session['code'] = $_GET['code'];
			$code = $_GET['code'];
			$globeAuth = $ultimatemember->globeapi->auth($ultimatemember->globelabs->getAppID(), $ultimatemember->globelabs->getSecretKey());
			$response = $globeAuth->getAccessToken($code);
			$wp_session['access_token'] = $response['access_token'];
			$wp_session['subscriber_number'] = $response['subscriber_number'];
			$data = array('user_name'=>$wp_session['user_login'], 'mobile_no'=>$response['subscriber_number'], 
					'code'=>$code,'token'=>$response['access_token'],'status'=>1);
			$ultimatemember->globelabs->insertData($data);
			$ultimatemember->user->auto_login($wp_session['user_id']);
			exit( wp_redirect( um_user_profile_url() ) );
		}

		if ( !get_option('users_can_register') && $mode == 'register' ) {
			$err = __('Registration is currently disabled','ultimatemember');
			echo "um-actions-misc line :59 : users can register";
		}
